...
"""
# how to know when process start, the starting file dir?
# like where do I start to go to SAM2_CHECKPOINT, e.g., pwd
...
# build sam 2
sam2_model = build_sam2(SAM2_CONFIG, SAM2_CHECKPOINT, device=device, current_dir=current_dir)
...
def referring_expression_segmentation(
    ...
    binary_mask = (sam2_masks[0] * 255).astype(np.uint8)

    # Save the binary mask as a PNG file
    mask_dir = os.path.basename(image_path).replace(".png", "_mask.png")
    mask_output_path = os.path.join(output_dir, mask_dir)
    ...
    label_annotator = sv.LabelAnnotator()
    annotated_frame = label_annotator.annotate(scene=annotated_frame, detections=detections, labels=labels)
    cv2.imwrite(os.path.join(output_dir, os.path.basename(image_path).replace(".png", "_florence2_referring_segmentation_box.jpg")), annotated_frame)
    
    mask_annotator = sv.MaskAnnotator()
    annotated_frame = mask_annotator.annotate(scene=annotated_frame, detections=detections)
    cv2.imwrite(os.path.join(output_dir, os.path.basename(image_path).replace(".png", "_florence2_referring_segmentation_box_with_mask.jpg")), annotated_frame)
    ...
    label_annotator = sv.LabelAnnotator()
    annotated_frame = label_annotator.annotate(scene=annotated_frame, detections=detections, labels=labels)
    cv2.imwrite(os.path.join(output_dir, os.path.basename(image_path).replace(".png", "_grounded_sam2_florence2_referring_box.jpg")), annotated_frame)
    
    mask_annotator = sv.MaskAnnotator()
    annotated_frame = mask_annotator.annotate(scene=annotated_frame, detections=detections)
    cv2.imwrite(os.path.join(output_dir, os.path.basename(image_path).replace(".png", "_grounded_sam2_florence2_referring_box_with_sam2_mask.jpg")), annotated_frame)
...
if __name__ == "__main__":
    ...
    if PIPELINE == "object_detection_segmentation":
        ...
        output_dir = referring_expression_segmentation(
                        image_path=IMAGE_PATH,
                        text_input=INPUT_TEXT
                    )
        print(f'Successfully save sam2 mask to "{output_dir}"')
